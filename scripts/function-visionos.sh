...
set explicitly. When compilation ends, libraries are created under the prebuilt folder.\n"
  ...
  display_help_options "  -x, --xcframework\t\tbuild xcframework bundles instead of framework bundles" "  -l, --lts			build lts packages to support sdk 1.0+ devices" "      --target=visionos sdk version\toverride minimum deployment target [1.0]"
...
disable_visionos_architecture_not_supported_on_detected_sdk_version() {
  ...
  arm64-simulator)
    ...
    # INTRODUCED IN VISIONOS SDK 1.0
    if [[ $(compare_versions "$DETECTED_VISIONOS_SDK_VERSION" "1.0") -ge 1 ]]; then
      local SUPPORTED=1
    else
      local SUPPORTED=0
    fi
...
disable_visionos_videotoolbox_on_not_supported_sdk_version() {

  # INTRODUCED IN VISIONOS SDK 1.0
  if [[ $(compare_versions "$VISIONOS_MIN_VERSION" "1.0") -lt 0 ]]; then
    set_library "visionos-videotoolbox" 0
    echo -e "INFO: Disabled visionos-videotoolbox which is not supported on visionOS SDK $VISIONOS_MIN_VERSION\n" 1>>"${BASEDIR}"/build.log 2>&1
  fi
}
...
get_common_cflags() {
  ...
  case ${ARCH} in
  arm64)
    echo "-fstrict-aliasing ${BITCODE_FLAGS} -DVISIONOS ${LTS_BUILD_FLAG}${BUILD_DATE} -Wno-incompatible-function-pointer-types -isysroot ${SDK_PATH}"
    ;;
  x86-64 | arm64-simulator)
    echo "-fstrict-aliasing -DVISIONOS ${LTS_BUILD_FLAG}${BUILD_DATE} -Wno-incompatible-function-pointer-types -isysroot ${SDK_PATH}"
    ;;
  esac
}
...
get_app_specific_cflags() {
  ...
  openh264 | x265)
    ...
    ;;
  sdl)
    APP_FLAGS="-DPIC -Wno-unused-function  -D__VISIONOS__"
    ;;
  shine)
    APP_FLAGS="-Wno-unused-function"
    ;;
...
initialize_prebuilt_visionos_folders() {
  if [[ -n ${FFMPEG_KIT_XCF_BUILD} ]]; then
    ...
    echo -e "DEBUG: Initializing universal directories and frameworks for xcframework builds\n" 1>>"${BASEDIR}"/build.log 2>&1
